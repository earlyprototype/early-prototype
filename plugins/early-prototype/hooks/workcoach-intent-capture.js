#!/usr/bin/env node
/**
 * Workcoach Intent Capture + Parrot.
 *
 ...
 *           <cwd>/.claude/workcoach-intent.txt (timestamped, session-tagged). A hook
 ...
 *
 * In the early-prototype suite this is registered for UserPromptSubmit (silent capture) only — the SessionStart parrot mode is intentionally not registered here, to avoid duplicating the coach session's start-prompt when both plugins are installed.
 *
 ...
 * workcoach-intent.txt ("<!-- session:<uuid> -->") makes "first message of THIS
...
const INTENT_FILENAME = 'workcoach-intent.txt';
const INTENT_TEXT_MAX = 1000;
const TAG = '[Workcoach-Intent]';
